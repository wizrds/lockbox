...
pub fn derive_request_dto_impl(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        ...
        where
            S: Send + Sync,
            lockbox_api::extractors::Json<#name>: axum::extract::FromRequest<S>,
        {
            type Rejection = <lockbox_api::extractors::Json<#name> as axum::extract::FromRequest<S>>::Rejection;

            async fn from_request(
                req: axum::http::Request<axum::body::Body>,
                state: &S,
            ) -> Result<Self, Self::Rejection> {
                Ok(lockbox_api::extractors::Json::<#name>::from_request(req, state).await?.0)
            }
        }
    };
...
pub fn derive_response_dto_impl(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let name = input.ident;
    ...
    let body = match status_expr {
        Some(expr) => quote! {
            (#expr, axum::response::Json(self)).into_response()
        },
        None => quote! {
