...
pub struct CliCommandNamespacesCreate {
    #[arg(help = "The Namespace name")]
    #[validate(length(max = 6))]
    pub name: String,
    #[arg(long, action = ArgAction::SetTrue, help = "Skip if the namespace already exists")]
...
#[async_trait]
impl Cmd for CliCommandNamespacesCreate {
    async fn update_ctx(&self, ctx: &mut Ctx) -> Result<()> {
        ...
        namespaces_ctx.skip_exists = self.skip_exists;
    ...
    async fn run(&self, ctx: &mut Ctx) -> Result<()> {
            ...
            .await
        {
            ...
            Err(ApiKeyServiceError::NamespaceAlreadyExists) if namespaces_ctx.skip_exists => Ok(()),
